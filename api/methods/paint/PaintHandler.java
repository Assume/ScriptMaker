...
public class PaintHandler
{
	...
	public static Map<String, PaintItem> resetAllPaintItems(
			Map<String, PaintItem> map)
	{
	...
	public static void drawTimeRan(Graphics g, long runtime)
	{
		...
		String[] infoArray = { "Runtime: " + SkillData.formatTime(vars.runtime) };
	...
	public static String[] getAllOther()
	{
	...
	public static String[] getAllExpItems()
	{
		String[] items = map.keySet().toArray(new String[map.size()]);
		List<String> list = new ArrayList<String>();
		for (int i = 0; i < items.length; i++)
	...
	public static void updateItem(String name, String extra)
	{
		if (map.containsKey(name))
		{
			PaintItem t = map.get(name);
			if(t instanceof GenericPaintItemString)
			{
				((GenericPaintItemString) t).updateData(extra);
			}
			else
			{
				t.updateData();
